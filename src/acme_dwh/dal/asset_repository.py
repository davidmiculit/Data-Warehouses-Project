...
from acme_dwh.dal.models import Asset
...
class AssetRepository(WarehouseRepository[Asset, str]):
    def __init__(self, session: Session | None = None) -> None:
        ...
        self._insert = self.session.prepare(
            "INSERT INTO asset (id, system_time, name, description, attributes, deleted) "
            "VALUES (?, ?, ?, ?, ?, ?)"
        )
        self._insert_id = self.session.prepare("INSERT INTO asset_ids (bucket, id) VALUES (?, ?)")
        self._select_latest = self.session.prepare(
            "SELECT id, system_time, name, description, attributes, deleted "
            "FROM asset WHERE id = ? LIMIT 1"
        )
        self._select_all = self.session.prepare(
            "SELECT id, system_time, name, description, attributes, deleted FROM asset WHERE id = ?"
        )
        self._select_ids = self.session.prepare("SELECT id FROM asset_ids WHERE bucket = ? LIMIT ?")

    def save(self, entity: Asset) -> Asset:
        self.session.execute(
            self._insert,
            (entity.id, entity.system_time, entity.name, entity.description,
             dict(entity.attributes or {}), entity.deleted),
        )
        self.session.execute(self._insert_id, (_BUCKET, entity.id))
        return entity

    def delete(self, key: str, valid_from: datetime | None = None) -> None:
        self.save(Asset(id=key, system_time=valid_from or utcnow(), deleted=True))

    def delete_all(self, key: str) -> None:
        self.session.execute("DELETE FROM asset WHERE id = %s", (key,))

    def find_latest(self, key: str) -> Asset | None:
        row = self.session.execute(self._select_latest, (key,)).one()
        return _to_asset(row) if row else None

    def find_all(self, key: str) -> list[Asset]:
        return [_to_asset(r) for r in self.session.execute(self._select_all, (key,))]

    def list_ids(self, limit: int = 20, offset: int = 0) -> list[str]:
        # bounded read of one ordered index partition, then slice (Cassandra has no OFFSET)
